...
export function PracticeRoom({ piece }: { piece: Piece }) {
  ...
  useEffect(() => { modeRef.current = mode; cursorRef.current = cursor; }, [mode, cursor]);
  useEffect(() => { speedRef.current = speed; metroRef.current = metro; }, [speed, metro]);

  const finish = useCallback((durationSec: number) => {
    ...
    void coachPractice({
      ...
      if (r.ok && r.tips.length) setReport((prev) => (prev ? { ...prev, tips: r.tips } : prev));
    });
  ...
  const tickPerform = useCallback(() => {
    ...
    if (beat > total + 0.6) { finish(t); return; }
  ...
  function startMetronome() {
    ...
    let beat = 0;
    const click = () => { getPiano().click(beat % piece.timeSignature.num === 0); beat++; };
    click();
  ...
  async function beginPlay() {
    ...
    if (modeRef.current === "perform") {
      ...
      if (first) setStatuses({ [first.id]: "current" });
    }
  }
  ...
  async function start() {
    ...
    const id = window.setInterval(() => {
      n -= 1;
      if (n <= 0) { window.clearInterval(id); void beginPlay(); }
      else setCount(n);
    }, 700);
  }
  ...
  const onDown = useCallback((midi: number, velocity = 0.85) => {
    ...
    if (modeRef.current === "follow") {
      const exp = expected[cursorRef.current];
      if (!exp || exp.midi == null) return;
      if (midi === exp.midi) {
        hitRef.current += 1;
        eventsRef.current.push({ midi, atMs: t, expectedIndex: cursorRef.current, status: "hit" });
        setStatuses((s) => ({ ...s, [exp.id]: "ok" }));
        const next = cursorRef.current + 1;
        cursorRef.current = next;
        setCursor(next);
        const nxt = expected[next];
        if (nxt) setStatuses((s) => ({ ...s, [nxt.id]: "current" }));
        else finish(t / 1000);
      } else {
        wrongRef.current += 1;
  ...
  async function connectMidi() {
    const { status } = await bindMidi({ onDown, onUp });
    setMidi(status);
  }

  const progress = expected.length === 0 ? 0 : Math.min(100, (Math.min(cursor, expected.length) / expected.length) * 100);
  const expectedMidi = expected[Math.min(cursor, expected.length - 1)]?.midi ?? null;
    ...
    <div className="flex min-h-dvh flex-col bg-bg">
      <header className="flex items-center justify-between gap-3 border-b border-border px-3 py-3 sm:px-6">
        <div className="flex min-w-0 items-center gap-2">
          <Button asChild variant="ghost" size="icon-sm" aria-label="返回">
            <Link to="/p/$id" params={{ id: piece.id }}><ChevronLeft /></Link>
          </Button>
        ...
        <div className="flex items-center gap-2">
          <Badge variant="paper" className="hidden sm:inline-flex"><Keyboard className="mr-1 size-3" />A–L 弹奏</Badge>
          {midi.connected.length > 0 && (
            <Badge variant="success"><Usb className="mr-1 size-3" />{midi.connected[0]}</Badge>
          )}
        </div>
      </header>
      <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 px-3 py-4 sm:px-6">
        <div className="flex flex-wrap items-center gap-2">
          {(["follow", "perform"] as const).map((m) => (
            <button key={m} type="button" disabled={phase === "playing" || phase === "count"} onClick={() => setMode(m)} className={cn("h-10 rounded-md px-3 text-sm font-medium", mode === m ? "bg-accent text-accent-fg" : "bg-elevated text-muted")}>
              {m === "follow" ? "跟弹" : "演奏"}
            </button>
          ))}
          <button type="button" onClick={() => setMetro((v) => !v)} className={cn("h-10 rounded-md px-3 text-sm", metro ? "bg-elevated text-fg" : "bg-elevated text-subtle")}>
            节拍器 {metro ? "开" : "关"}
          </button>
          <div className="flex min-w-36 flex-1 items-center gap-2 sm:max-w-xs">
            ...
            <Slider min={0.5} max={1.2} step={0.05} value={[speed]} disabled={phase === "playing"} onValueChange={(v) => setSpeed(v[0] ?? 1)} />
          </div>
          <Button variant="outline" size="sm" onClick={() => void connectMidi()}><Usb className="size-3.5" />MIDI</Button>
        </div>
        <div className="relative">
          <StaffView notes={piece.notes} timeSignature={piece.timeSignature} keySignature={piece.key} cursorIndex={phase === "playing" || phase === "paused" ? cursor : undefined} statuses={statuses} />
          {phase === "count" && (
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-muted tabular-nums">
            {Math.min(cursor, expected.length)}/{expected.length}{phase === "playing" && mode === "perform" ? ` · ${formatDuration(elapsed)}` : ""}
          </span>
        </div>
        <div className="flex flex-wrap gap-2">
          {phase === "idle" || phase === "done" ? (
            <Button onClick={() => void start()}><Play className="size-4" />开始</Button>
          ) : phase === "paused" ? (
            <Button onClick={() => void beginPlay()}><Play className="size-4" />继续</Button>
          ) : phase === "playing" ? (
            <Button variant="secondary" onClick={pause}><Pause className="size-4" />暂停</Button>
          ) : null}
          <Button variant="ghost" onClick={reset}><RotateCcw className="size-4" />重来</Button>
        </div>
        {report && phase === "done" && (
          <div className="stagger-in rounded-xl bg-surface p-5 shadow-[var(--shadow-border)]">
            ...
            <p className="mt-4 text-sm text-muted">命中 {report.hit} · 错音 {report.wrong} · 漏音 {report.missed}{report.extra ? ` · 多余 ${report.extra}` : ""}</p>
            {coach && <p className="mt-3 font-serif text-base text-fg">{coach}</p>}
            <ul className="mt-3 space-y-1.5">{report.tips.map((t) => (<li key={t} className="text-sm text-muted">{t}</li>))}</ul>
            <div className="mt-5 flex flex-wrap gap-2">
              <Button onClick={() => void start()}>再练一次</Button>
              <Button asChild variant="secondary"><Link to="/">返回曲库</Link></Button>
            </div>
          </div>
        )}
      </div>
      <div className="sticky bottom-0 border-t border-border bg-bg/95 px-3 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-6">
        <div className="mx-auto max-w-6xl overflow-x-auto">
          <PianoKeyboard active={active} expected={phase === "playing" && mode === "follow" ? expectedMidi : null} wrong={wrongKeys} onDown={(m) => onDown(m)} onUp={onUp} />
        </div>
      </div>
    </div>
  );
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (<div><p className="text-xs text-muted">{label}</p><p className="font-medium tabular-nums">{value}</p></div>);
}
